...
export const createOrder = async (data,access_token) => {
  const res = await axiosJWT.post(`http://localhost:3001/api/order/create/${data?.user}`, data, {
      headers: {
...
export const cancelOrder = async (id, access_token, orderItems, userId ) => {
  ...
  const res = await axiosJWT.delete(`http://localhost:3001/api/order/cancel-order/${userId}`, {data}, {
      headers: {
          token: `Bearer ${access_token}`,
      }
  })
